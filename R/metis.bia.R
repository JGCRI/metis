...
#' @param dirbiaOutputs Default =paste(getwd(),"/dataFiles/grids/bia/biaOutputs",sep=""),
...
metis.prepGrid<- function(biaFolder="NA",
                          biaFiles="NA",
  ...
  NULL -> lat -> lon -> latitude -> longitude -> aez_id -> region_id ->X..ID->
    ...
    owner->region->regionsSelect->rowid->country_long->gppd_idnr


  #------------------
  # Create Folders if needed
  #------------------
  if (!dir.exists(dirbiaOutputs)){
    dir.create(dirbiaOutputs)}

  if (!dir.exists(paste(dirbiaOutputs, "/biadiagnostics",sep=""))){
    dir.create(paste(dirbiaOutputs, "/biadiagnostics",sep=""))}
    ...
    if(F){
      ...
      for(regioni in regionsSelect){
        ...
        metis.printPdfPng(figure=ggplot(data = gridR, aes(fill = data_source, x = class1, y = est_installed_capacity))+geom_bar(position = "dodge", stat="identity"),
                          dir=paste(dirbiaOutputs, "/biadiagnostics",sep=""),filename=fname,figWidth=9,figHeight=7,pdfpng="png")
