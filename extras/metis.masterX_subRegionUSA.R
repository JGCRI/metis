...
if (!dir.exists(paste(getwd(),"/outputs/",analysisFolder,sep=""))){
  dir.create(paste(getwd(),"/outputs/",analysisFolder,sep=""))}
if (!dir.exists(paste(getwd(),"/outputs/",analysisFolder,"/gis",sep=""))){
  dir.create(paste(getwd(),"/outputs/",analysisFolder,"/gis",sep=""))}
if (!dir.exists(paste(getwd(),"/outputs/",analysisFolder,"/gis",sep=""))){
  dir.create(paste(getwd(),"/outputs/",analysisFolder,"/gis",sep=""))
  gisFolder=paste(getwd(),"/outputs/",analysisFolder,"/gis",sep="")}
...
examplePolyFile_i<-paste("ne0_USMainLand_cropped",sep="")
countryNE0=rgdal::readOGR(dsn=examplePolyFolder_i,layer=examplePolyFile_i,use_iconv=T,encoding='UTF-8')
head(countryNE0@data)
countryNE0<-countryNE0[(countryNE0$ADMIN==countryName),]
head(countryNE0@data)
plot(countryNE0)
projX<-proj4string(countryNE0)
writeOGR(obj=countryNE0, dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""), layer=paste(shortName,"_NE0",sep=""), driver="ESRI Shapefile", overwrite_layer=TRUE)

#--------------
# State shapes
#--------------
if(!file.exists(paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_NE1.shp",sep=""))){
  ...
  countryNE1<-raster::crop(countryNE1,countryNE0)
  countryNE1@data <- droplevels(countryNE1@data)
  head(countryNE1@data)
  plot(countryNE1)
  countryNE1<-spTransform(countryNE1,CRS(projX))
  writeOGR(obj=countryNE1, dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""), layer=paste(shortName,"_NE1",sep=""), driver="ESRI Shapefile", overwrite_layer=TRUE)
  metis.map(dataPolygon=countryNE1,fillColumn = "postal",printFig=F, facetsON = F, labels=T, legendStyle = "cat")
}else{countryNE1=readOGR(dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""),
                         layer=paste(shortName,"_NE1",sep=""),use_iconv=T,encoding='UTF-8')
metis.map(dataPolygon=countryNE1,fillColumn = "postal",printFig=F, facetsON = F, labels=T, legendStyle = "cat")}

#--------------
# County
#--------------
if(!file.exists(paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_County.shp",sep=""))){
  # Natural earth level 1 admin boundaries
  countryCounty<-readOGR(dsn=paste(getwd(),"/dataFiles/gis/metis/USA/cb_2018_us_county_20m",sep=""),
               layer="cb_2018_us_county_20m",use_iconv=T,encoding='UTF-8')
   # subset any islands or regions not wanted
  head(countryCounty@data)
  countryCounty<-raster::crop(countryCounty,countryNE0)
  countryCounty@data <- droplevels(countryCounty@data)
  head(countryCounty@data)
  plot(countryCounty)
  countryCounty<-spTransform(countryCounty,CRS(projX))
  writeOGR(obj=countryCounty, dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""), layer=paste(shortName,"_County",sep=""), driver="ESRI Shapefile", overwrite_layer=TRUE)
  metis.map(dataPolygon=countryCounty,fillColumn = "STATEFP",printFig=F, facetsON = F, labels=T, legendStyle = "cat")
}else{countryNE1=readOGR(dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""),
                         layer=paste(shortName,"_County",sep=""),use_iconv=T,encoding='UTF-8')
metis.map(dataPolygon=countryNE1,fillColumn = "STATEFP",printFig=F, facetsON = F, labels=T, legendStyle = "cat")}


#--------------
# GCAN BASIN
#--------------
if(!file.exists(paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_GCAMBasin.shp",sep=""))){  # GCAM Basins
  ...
  writeOGR(obj=countryGCAMBasin, dsn=paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_GCAMBasin.shp",sep=""), layer=paste(countryName,"GCAMBasin",sep=""), driver="ESRI Shapefile", overwrite_layer=TRUE)
  metis.map(dataPolygon=countryGCAMBasin,fillColumn = "basin_name",printFig=F,facetsON = F, labels=T, legendStyle = "cat")
}else{countryGCAMBasin=readOGR(dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""),
                               layer=paste(shortName,"_GCAMBasin",sep=""),use_iconv=T,encoding='UTF-8')
metis.map(dataPolygon=countryGCAMBasin,fillColumn = "basin_name",printFig=F, facetsON = F, labels=T, legendStyle = "cat")}
...
# Hydro Basins Lev 2
#--------------
if(!file.exists(paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_hydroBasinLev2.shp",sep=""))){  # GCAM Basins

  hydroBasinLev2<-readOGR(dsn=paste(getwd(),"/dataFiles/gis/metis/subbasin_hydrobasin",sep=""),
                     layer="hydrobasins_level_2",use_iconv=T,encoding='UTF-8')
  hydroBasinLev2<-spTransform(hydroBasinLev2,CRS(projX))
  countryhydroBasinLev2<-raster::crop(hydroBasinLev2,countryNE1)
  countryhydroBasinLev2@data <- droplevels(countryhydroBasinLev2@data)
  head(countryhydroBasinLev2@data)
  plot(countryhydroBasinLev2)
  writeOGR(obj=countryhydroBasinLev2, dsn=paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_hydroBasinLev2.shp",sep=""), layer=paste(countryName,"hydroBasinLev2",sep=""), driver="ESRI Shapefile", overwrite_layer=TRUE)
  metis.map(dataPolygon=countryhydroBasinLev2,fillColumn = "HYBAS_ID",printFig=F,facetsON = F, labels=T, legendStyle = "cat")
}else{countryhydroBasinLev2=readOGR(dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""),
                               layer=paste(shortName,"_hydroBasinLev2",sep=""),use_iconv=T,encoding='UTF-8')
metis.map(dataPolygon=countryhydroBasinLev2,fillColumn = "HYBAS_ID",printFig=F, facetsON = F, labels=T, legendStyle = "cat")}

#--------------
# Hydro Basins Lev 3
#--------------
if(!file.exists(paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_hydroBasinLev3.shp",sep=""))){  # GCAM Basins

  hydroBasinLev3<-readOGR(dsn=paste(getwd(),"/dataFiles/gis/metis/subbasin_hydrobasin",sep=""),
                          layer="hydrobasins_level_3",use_iconv=T,encoding='UTF-8')
  hydroBasinLev3<-spTransform(hydroBasinLev3,CRS(projX))
  countryhydroBasinLev3<-raster::crop(hydroBasinLev3,countryNE1)
  countryhydroBasinLev3@data <- droplevels(countryhydroBasinLev3@data)
  head(countryhydroBasinLev3@data)
  plot(countryhydroBasinLev3)
  writeOGR(obj=countryhydroBasinLev3, dsn=paste(gisFolder,"/shapefiles_",shortName,"/",shortName,"_hydroBasinLev3.shp",sep=""), layer=paste(countryName,"hydroBasinLev3",sep=""), driver="ESRI Shapefile", overwrite_layer=TRUE)
  metis.map(dataPolygon=countryhydroBasinLev3,fillColumn = "HYBAS_ID",printFig=F,facetsON = F, labels=T, legendStyle = "cat")
}else{countryhydroBasinLev3=readOGR(dsn=paste(gisFolder,"/shapefiles_",shortName,sep=""),
                                    layer=paste(shortName,"_hydroBasinLev3",sep=""),use_iconv=T,encoding='UTF-8')
metis.map(dataPolygon=countryhydroBasinLev3,fillColumn = "HYBAS_ID",printFig=F, facetsON = F, labels=T, legendStyle = "cat")}

# Plot Mpas
# State, County, GCAM BAsins, HydroBasins Lev 3, HydroBasins Lev 2
metis.map(dataPolygon=countryNE1,fileName="states",dirOutputs=analysisFolder,folderName="Maps",fillColumn = "postal",printFig=T, facetsON = F, labels=F, legendStyle = "cat")
metis.map(dataPolygon=countryCounty,fileName="county",dirOutputs=analysisFolder,folderName="Maps",fillColumn = "STATEFP",printFig=T, facetsON = F, labels=F, legendStyle = "cat")
metis.map(dataPolygon=countryGCAMBasin,fileName="GCAMBasin",dirOutputs=analysisFolder,folderName="Maps",fillColumn = "basin_name",printFig=T, facetsON = F, labels=F, legendStyle = "cat")
metis.map(dataPolygon=countryhydroBasinLev2,fileName="hydrobasLev2",dirOutputs=analysisFolder,folderName="Maps",fillColumn = "HYBAS_ID",printFig=T, facetsON = F, labels=F, legendStyle = "cat")
metis.map(dataPolygon=countryhydroBasinLev3,fileName="hydrobasLev3",dirOutputs=analysisFolder,folderName="Maps",fillColumn = "HYBAS_ID",printFig=T, facetsON = F, labels=F, legendStyle = "cat")
