...
# Tethys
tethysFolders=c(paste(getwd(),"/dataFiles/grids/tethys/Reference",sep=""),
                paste(getwd(),"/dataFiles/grids/tethys/Impacts",sep=""),
                paste(getwd(),"/dataFiles/grids/tethys/Policy",sep=""))
tethysScenarios=c("Ref", "HADGEM2Rcp8p5", "Policy")
...
paramScenarios_i <- paramScenarios_i[!grepl("griddedScarcity",paramScenarios_i)];paramScenarios_i
...
scenRefDiffIndv_i = list(param=list(c("tethysWatWithdraw_total")),
                         #scenRef=list(c("GFDL-ESM2M_rcp8p5")),
                         #scenDiff=list(c("GFDL-ESM2M_rcp2p6")),
                         scenIndv=list(c("Ref","HADGEM2Rcp8p5","Policy"))
); scenRefDiffIndv_i
...
#xRange_i= seq(from=2050,to=2050,by=10)
xRange_i= c(2010,2050)
multiFacetsOn_i=T  # Multifacets ignores all the scenarios given above and looks for a multiRef scenario in all the data provided
refMultiA_i="GFDL-ESM2M"
refMultiB_i="rcp2p6"
